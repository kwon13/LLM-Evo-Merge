...
class ModelGenome:
    ...
    def __init__(
        ...
        n_param_sets = len(definition.filters or []) + 1
        n_models = len(definition.models)
        n_params = len(METHOD_PARAM_MAPS[definition.merge_method])
        self.dimensions = (n_layer_groups, n_models, n_param_sets, n_params)
    ...
    def initial_genotype(self, population_size) -> torch.Tensor:
        """Generate an initial genotype for the given number of layers."""
        return torch.rand(population_size, *self.dimensions)
